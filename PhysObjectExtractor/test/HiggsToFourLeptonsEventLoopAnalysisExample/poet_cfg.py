...
#---- Example of a very basic home-made filter to select only events of interest
#---- The filter can be added to the running path below if needed but is not applied by default
process.myleptonfilter = cms.EDFilter('Simple4LeptonFilter',
                                   InputCollectionMuons = cms.InputTag("muons"),
...
#if doPat:
#	process.p = cms.Path(process.myleptonfilter+process.patDefaultSequence+process.myevents+process.myelectrons+process.mymuons+process.myphotons+process.myjets+process.mymets+process.mytaus+process.mytrigEvent+process.mypvertex+process.mytracks+process.mygenparticle+process.mytriggers)
#else: 
...
process.p = cms.Path(process.myleptonfilter+process.myevents+process.myelectrons+process.mymuons+process.mymets+process.mytaus+process.mypvertex+process.mygenparticle+process.mytriggers)
