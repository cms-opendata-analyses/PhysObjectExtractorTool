...
//create trigger histograms
	trighist_e = fs->make <TH1D>("trighist_energy", "obj energy", 100, 0, 5000);
	trighist_pt = fs->make <TH1D>("trighist_pt", "obj pt ", 100,0,5000 );
	trighist_px = fs->make <TH1D>("trighist_px", "obj px ", 100, 0, 5000 );
	trighist_py = fs->make <TH1D>("trighist_py", "obj py ", 100, 0, 5000 );
	trighist_pz = fs->make <TH1D>("trighist_pz", "obj pz ", 100, 0, 5000 );
	trighist_eta = fs->make <TH1D>("trighist_eta", "obj eta ", 100, 0, 5000 );
	trighist_phi = fs->make <TH1D>("trighist_phi", "obj phi ", 100, 0, 5000 );
	trigobjhisto = fs->make <TH1D>("trigobjchisto", "trig obj histo", 100, 0, 5000);
...
TriggObjectAnalyzer::analyzeTriggObject(const edm::Event& iEvent, const edm::Handle<trigger::TriggerEvent> &trigEvent, const edm::InputTag &trigEventTag_)
{
    ...
    for(trigger::Keys::const_iterator keyIt=trigKeys.begin();keyIt!=trigKeys.end();++keyIt){ 
      ...
      //eta,phi,pt,mass,p,px,py,pz,et,energy accessors
	    ...
	    trighist_e->Fill(trigobj.energy());
	    trighist_pt->Fill(trigobj.pt());
	    trighist_px->Fill(trigobj.px());
	    trighist_py->Fill(trigobj.py());
	    trighist_pz->Fill(trigobj.pz());
	    trighist_eta->Fill(trigobj.eta());
	    trighist_phi->Fill(trigobj.phi());
